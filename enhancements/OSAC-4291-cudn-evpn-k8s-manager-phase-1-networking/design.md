...
1. **Fabric manager** (Netris) runs when Subnet is created:
   ...
   - Writes output to ConfigMap: `{l2_vni, l3_vni, l2_route_target, l3_route_target, fabric_reserved_range}`
...
3. **K8s manager** (cudn_evpn) provisions CUDN (first Subnet only):
   ...
   - Passes the Netris-provided L2 and L3 route targets to CUDN; the k8s manager does not calculate route targets
...
- **ClusterUserDefinedNetwork** (OVN-Kubernetes CRD): k8s manager creates with EVPN transport, macVRF/ipVRF VNI, and the route-target values returned by the fabric manager
...
func extractExtraVarsFromConfigMap(ctx context.Context, configMapName string) (map[string]interface{}, error) {
    ...
    // Fabric manager writes: {"l2_vni": 14, "l3_vni": 11, "l2_route_target": "65000:14", "l3_route_target": "65000:11", "fabric_reserved_range": "200.200.1.1/32,200.200.1.100-200.200.1.200"}
...
- name: Extract VNI and reserved range from Netris response
  ansible.builtin.set_fact:
    ...
    l2_route_target: "{{ vnet_result.vnet.l2_route_target }}"
    l3_route_target: "{{ vpc_result.vpc.l3_route_target }}"
...
- name: Publish VNI data for k8s manager via ConfigMap
        ...
        extra_vars: |
          {
            ...
            "l2_route_target": "{{ l2_route_target }}",
            "l3_route_target": "{{ l3_route_target }}",
  ...
  # Netris is the route-target source of truth; the k8s manager consumes the returned values
...
- Netris returns the L2 and L3 route targets; the k8s manager passes them to CUDN without calculating them
...
```yaml
---
- name: Validate required VNI, route-target, and reserved-range output from fabric job
  ansible.builtin.assert:
    that:
      - osac_job_vars.extra_vars.l2_vni is defined
      - osac_job_vars.extra_vars.l3_vni is defined
      - osac_job_vars.extra_vars.l2_route_target is defined
      - osac_job_vars.extra_vars.l3_route_target is defined
      - osac_job_vars.extra_vars.fabric_reserved_range is defined
    fail_msg: "Fabric job must provide l2_vni, l3_vni, l2_route_target, l3_route_target, and fabric_reserved_range."

- name: Extract VNI values from extra_vars
  ansible.builtin.set_fact:
    l2_vni: "{{ osac_job_vars.extra_vars.l2_vni }}"
    l3_vni: "{{ osac_job_vars.extra_vars.l3_vni }}"
    l2_route_target: "{{ osac_job_vars.extra_vars.l2_route_target }}"
    l3_route_target: "{{ osac_job_vars.extra_vars.l3_route_target }}"
      ...
      spec:
        ...
        network:
          ...
          evpn:
            ...
            macVRF:
              vni: "{{ l2_vni | int }}"
              routeTarget: "{{ l2_route_target }}"
            ipVRF:
              vni: "{{ l3_vni | int }}"
              routeTarget: "{{ l3_route_target }}"
...
- Route targets are read from the fabric manager output and supplied to CUDN; no route-target calculation occurs in the k8s manager
...
| **cudn_evpn coupled to Netris-specific outputs** — k8s manager expects Netris VPC/VNet concepts, won't work with other fabric managers | Generic output contract (`l2_vni`, `l3_vni`, `l2_route_target`, `l3_route_target`, `fabric_reserved_range`) defines the fabric→k8s data flow. Any fabric manager can provide this contract without k8s manager route-target calculation. Netris is the first implementation. |
...
The executable, reviewable Phase 1 plan is maintained in
[testplan.md](testplan.md). It covers manager registration and capabilities,
sequential fabric-to-CUDN provisioning, supported subnet and VM placement,
deletion and dependency guards, concurrency and isolation, and explicitly
unsupported Phase 1 behavior. Shared networking contracts are covered by the
[Unified Networking test plan](../OSAC-1433-unified-networking/testplan.md).
## Graduation Criteria
