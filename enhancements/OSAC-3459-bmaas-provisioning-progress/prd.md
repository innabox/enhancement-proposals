...
- While a bare metal instance is actively provisioning or deprovisioning, the progress view auto-refreshes on a bounded ~5-second interval without requiring user action, and stops polling only once the instance's current activity is finished — a successfully provisioned instance that is resting at ready, an instance whose provisioning failed, or an instance whose teardown has completed (released) or failed. Deleting a previously ready instance re-activates polling for the teardown timeline: a past "ready" does not suppress deprovisioning progress. Users see backend progress reflected within single-digit seconds without reloading the page. [Clarify: R1.Q2]
