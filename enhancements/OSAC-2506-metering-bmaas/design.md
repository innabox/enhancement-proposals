...
**BMaaS event ordering contract:** Because `Events.Watch` does not guarantee delivery order, BMaaS uses the existing per-resource `BareMetalInstance.metadata.version` on `OBJECT_CREATED` and `OBJECT_UPDATED` payloads as its monotonic ordering value. Fulfillment increments this field on normal object updates and preserves it in reconciliation snapshots and durable-history records. The projection stores the highest contiguous metadata version it has applied. `Event.id` remains the stable event identity for deduplication; it is not an ordering value or replay cursor. `OBJECT_DELETED` is a terminal exception: the current fulfillment deletion path can emit the deletion notification with the same metadata version as the preceding object event, so deletion bypasses the numeric version comparison and is guarded by its stable event ID, required `deletion_completion_time`, and a deletion tombstone. Late normal events for a deleted resource are ignored.
...
| `LastMetadataVersion`    | Highest contiguous per-resource `BareMetalInstance.metadata.version` applied to this projection; persisted with the projection and its outbox records                  |
...
Before resolving a normal transition, the handler compares the incoming `metadata.version` with `LastMetadataVersion` while holding the projection lock. A version less than or equal to the stored value is acknowledged without changing the projection or emitting events. A version greater than the next expected value is held and replayed from the durable fulfillment history; it is never applied speculatively, because doing so could skip a billing boundary. `OBJECT_DELETED` does not advance `LastMetadataVersion`; it requires the completion timestamp and is deduplicated through its event ID and deletion tombstone. The accepted normal event advances `LastMetadataVersion` in the same transaction as the projection update, processed-event record, and outbox records. A projection without an established version is initialized by startup reconciliation or durable replay before live events are applied.
...
- BMaaS event handling ignores duplicate and stale `metadata.version` values without changing the projection or emitting events
- BMaaS event handling holds a metadata-version gap, replays the missing durable history, and applies normal events only in contiguous version order
