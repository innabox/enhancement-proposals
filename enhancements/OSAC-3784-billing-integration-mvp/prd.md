...
- **Tenant-to-billing-account lifecycle** — OSAC provisions and links billing accounts as tenants are created and deleted, including 1:N account-to-tenant sharing. On deletion, only the deleted tenant's records and its association with the account are removed; a shared account is retained until its last tenant is removed and pending charges settle, so one tenant's deletion never removes or hides another tenant's invoices or charges. Each account is provisioned with a single immutable base currency validated as an active ISO-4217 code.
