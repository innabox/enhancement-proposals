...
isolation, security, IP allocation, DNAT, and SNAT. In K8s-only mode, the K8s
Manager provides the supported equivalent for every resource except
NATGateway.
...
| `NetworkClass.spec.metallb_vip_prefix_length` | `int32`, conditionally required | Required when the deployment advertises CaaS/MetalLB VIP support; no universal default exists. It must be a valid IPv4 prefix more specific than each participating Subnet prefix, and the reserved range must remain inside the Subnet. It is not required for deployments that do not provide CaaS VIP allocation. |
...
all resource types use the K8s manager's supported OVN networking domain.
...
creates an overlay on each hosting cluster and bridges it to that segment.
...
message VirtualNetworkSpec {
  string implementation_strategy = 1; // provider-resolved/private, immutable
...
test must not replace a shared negative test with a service-specific happy
path: every supported operation needs an acceptance test and every explicitly
unsupported operation or value needs a rejection or fail-closed test.

The test suite must distinguish three kinds of behavior:

1. **Accepted and persisted:** the request is valid, the complete resolved
   network spec is stored, and reconciliation is allowed to begin.
2. **Accepted but asynchronous:** the request is valid and may create an
   internal Pending dependency graph, after which controllers requeue until
   all required prerequisites are Ready/Allocated. Only the internal default
   and auto-provisioning transactions may use this behavior.
3. **Rejected or fail-closed:** the request or backend result is outside the
   supported contract. No invalid user resource, partial allocation, or
   premature backend operation may be left behind.

### Test environments and fixtures

The shared suite uses progressively more realistic environments:

- **Unit environment:** fulfillment-service packages with in-memory request
  objects, fake repositories, fake authorization context, fake manager
  capability registrations, fake dispatcher results, and deterministic
  status fixtures. Controller unit tests use fake Kubernetes clients. These
  tests must not depend on a real cluster or fabric.
- **Service integration environment:** fulfillment-service with real
  ephemeral PostgreSQL, the real protovalidate and OPA/RBAC layers, and the
  real private/public handlers. Database migrations and delete-protection
  triggers run against that database. An envtest or Kind API server runs the
  CRDs, CEL validation, admission, and controller reconciliation. Managers
  are contract-compatible fakes that can return Pending, Ready, Failed,
  unsupported, malformed, and transient-error responses.
- **Cross-service integration environment:** fulfillment-service, the
  relevant operator(s), dispatcher/job objects, feedback controllers, and
  Catalog resolution run together. External infrastructure remains mocked,
  but asynchronous job completion, status propagation, retries, restarts,
  finalizers, and resource cleanup are real.
- **E2E environment:** the supported connected single-hub deployment with
  real authentication, tenant isolation, fulfillment-service, operators,
  dispatcher/AAP, and the manager implementation being graduated. Where a
  feature claims dataplane behavior, the test uses real fabric/Kubernetes
  connectivity and verifies traffic, not only object status. The matrix must
  include both combined fabric+K8s-manager and K8s-only deployments where
  the capability contract says both are supported.

### Unit tests — shared contract

The unit suite must exhaustively enumerate the following accepted and rejected
inputs.

#### Request shape, formats, and presence

- Accept every required message and field in the documented create request.
- Reject missing required messages, empty required lists, unspecified enum
  values, unknown enum values, malformed references, unknown fields, and
  invalid oneof combinations.
- Accept canonical IPv4 addresses and IPv4 network CIDRs with host bits zero.
- Reject IPv6, dual-stack, host bits in a network CIDR, malformed prefixes,
  address strings with a CIDR suffix where an address is required, and
  non-canonical encodings.
- Accept valid RFC 3339/`google.protobuf.Timestamp` status values and reject
  malformed, non-UTC, or otherwise invalid timestamp representations where
  timestamps are exposed.
- Test omitted optional fields separately from explicit empty strings,
  explicit empty repeated fields, zero values, and explicit `false` presence.
- Verify that invalid input is rejected rather than normalized into a valid
  value.

#### NetworkClass and manager capabilities

- Accept a fabric-only NetworkClass when the fabric manager advertises the
  complete supported resource surface.
- Accept a K8s-only NetworkClass when the K8s manager advertises the complete
  supported surface for the requested resources.
- Accept a combined fabric-manager/K8s-manager NetworkClass.
- Reject a NetworkClass with neither manager.
- Reject an unregistered manager, an unknown capability, a capability with an
  unsupported operation, and a manager/resource combination that cannot
  cover the requested resource.
- Verify manager resolution from `manages`; the tenant cannot select a
  different manager through a VirtualNetwork or workload attachment.
- Verify the implementation strategy is derived from the resolved managers,
  not accepted as tenant input or as a substitute for `network_class`.
- Accept CaaS capability configuration without a K8s manager where the
  design supports BM-based CaaS and MetalLB VIP allocation.
- Reject VM placement without a K8s manager.
- Reject NATGateway creation when the selected manager capability does not
  advertise NATGateway support, including K8s-only OVN mode.
- Reject conditional CaaS configuration without the required
  `metallb_vip_prefix_length`; accept configurations where the capability is
  not advertised without inventing a universal default.
- Verify provider-only NetworkClass and capability fields cannot be supplied
  or changed by a tenant.

#### Common resource fields and relationships

- Accept valid VirtualNetwork, Subnet, SecurityGroup, ExternalIPPool,
  ExternalIP, ExternalIPAttachment, and NATGateway creates.
- Reject wrong-type references, missing references, cross-tenant references,
  cross-project references, non-Ready references, and references with the
  wrong parent VirtualNetwork.
- Accept a Subnet contained by its parent VirtualNetwork; reject a Subnet
  outside the parent, equal to the parent, or overlapping a sibling.
- Accept overlapping VirtualNetwork CIDRs for different tenants where fabric
  isolation is the documented boundary; reject overlap that violates the
  within-VirtualNetwork Subnet contract.
- Reject overlapping ExternalIPPool ranges within the deployment.
- Enforce one NATGateway per VirtualNetwork and one ExternalIP consumer per
  ExternalIP.
- Require `SecurityGroup.rules` for tenant-created SecurityGroups and reject
  empty tenant-created rule sets. Accept an empty system-created fallback
  SecurityGroup only when its deployment baseline permit policy is present.
- Validate every SecurityGroup rule independently: action, direction,
  protocol, port requirements/range, direction-specific CIDR, duplicates,
  and conflicting equal-specificity rules.
- Verify deployment baseline permit behavior remains present and is not
  confused with the tenant fallback SecurityGroup.
- Verify positive rule evaluation: the most-specific matching tenant rule
  wins, exact protocol outranks `any`, exact port outranks an omitted port,
  and the least-specific deployment permit remains the fallback.
- Verify ExternalIPAttachment target oneof cardinality, target readiness,
  endpoint enum compatibility, duplicate binding rejection, and the
  Allocated ExternalIP precondition.
- Accept Compute/BM ExternalIPAttachments with `UNSPECIFIED` endpoint and
  Cluster attachments with exactly `API` or `INGRESS`; reject every other
  target/endpoint combination.
- Verify NATGateway requires a Ready VirtualNetwork and an Allocated,
  unconsumed ExternalIP.
- Verify manager-reported SNAT allocation/release/status transitions and
  reject a malformed or premature NATGateway Ready response.

#### Attachment cardinality and defaulting

- Accept omitted and explicitly empty workload attachment input according to
  the service-specific contract.
- Accept a partial attachment and fill only the missing Subnet or
  SecurityGroup field from the tenant default.
- Preserve a supplied Subnet and non-empty SecurityGroup list exactly.
- Reject a supplied reference that is invalid rather than repairing it with a
  default.
- Reject duplicate SecurityGroup references.
- Reject more than one Compute or BM attachment and any explicit
  `primary: false` under the single-interface contracts.
- Accept an omitted or `true` primary value for the sole Compute/BM entry.
- Accept a singular Cluster attachment and reject any attempted repeated or
  multi-attachment representation.
- Verify all resolved attachments use one VirtualNetwork and all resolved
  references are Ready before a direct tenant create is persisted.
- Verify only internal default/auto-provisioning creates may persist a
  Pending dependency graph; direct tenant requests with unresolved
  dependencies fail before persistence.

#### Allocation, capacity, and transactionality

- Allocate an ExternalIP only from a Ready IPv4 pool with capacity.
- Verify deterministic pool selection among equal-capacity pools.
- Reject arbitrary tenant-selected ExternalIP addresses.
- Reject pool exhaustion and verify that the parent, ExternalIP,
  ExternalIPAttachment, and capacity reservation are all rolled back.
- Exercise uniqueness races for Subnets, rules, NATGateways, and ExternalIP
  consumers; exactly one transaction succeeds and the losing transaction
  leaves no partial objects.
- Verify idempotent retries adopt only an object with the exact same owner,
  parent, immutable identity, and network spec.
- Reject retries that find a mismatched existing object instead of silently
  adopting it or creating a duplicate.

#### Operations, immutability, and deletion

- Accept create, read, and list for every supported resource in the permitted
  scope.
- Verify read/list filtering never exposes another tenant's resources,
  references, or status.
- Reject update, patch, replace, and field-mask changes to every network-owned
  field, including nested fields, list length/order, rule lists, references,
  `primary`, interface, endpoint, and auto-external switches.
- Accept controller changes to status, conditions, timestamps, discovered IPs,
  and finalizers only; reject status as an alternate spec-input path.
- Reject delete while reverse references or child resources remain.
- Accept leaf-first deletion and verify the documented finalizer order.
- Verify auto-created ExternalIPAttachment is deleted before its ExternalIP.
- Verify a parent delete is never treated as an implicit unsafe cascade.

#### Direct CR/API bypass and backend responses

- Submit invalid CRs directly to the hub API and verify CRD/CEL or controller
  rejection matches the fulfillment-service contract.
- Attempt to bypass immutability, cardinality, readiness, or scope checks by
  writing directly to a CR.
- Reject a caller attempt to set Ready or other controller-owned status.
- Accept Pending → Ready and Pending → Failed manager transitions only through
  the defined state machine.
- Reject malformed manager output, IPv6 output, an IP outside the selected
  Subnet, an invalid state transition, or a manager claim of an unsupported
  operation.

### Integration tests — shared component boundaries

The integration suite must prove that the same contract survives each
boundary, not only that an individual validator returns the right result.

- Exercise public API, private API, REST gateway, and direct CR submission for
  equivalent valid and invalid requests.
- Verify authentication, provider/tenant authorization, reference visibility,
  and cross-tenant isolation with multiple tenants and projects.
- Verify the complete create transaction: validation, default resolution,
  readiness checks, capacity reservation, persistence, and creation of
  internal auto-provisioned children.
- Inject a failure at each stage and verify rollback leaves no parent,
  child, capacity reservation, duplicate rule, or backend request.
- Verify PostgreSQL uniqueness and referential-integrity protections under
  concurrent create/delete operations.
- Verify update, patch, replace, direct CR mutation, and nested field-mask
  attempts are rejected consistently.
- Verify admission/CEL protects the hub CR when fulfillment-service is
  bypassed and controller validation protects it when an invalid object is
  admitted through a test fixture.
- Verify a Pending dependency causes a condition and requeue, while a direct
  user request referencing that Pending object is rejected rather than
  persisted.
- Verify managers are dispatched only after all readiness and capability
  preconditions pass, and that a manager failure cannot mark the resource
  Ready.
- Verify controller restart at every asynchronous boundary is idempotent and
  does not create duplicate jobs, segments, IPs, rules, attachments, or
  finalizers.
- Verify deletion guards across the complete graph:
  workload → Subnet/SecurityGroup, ExternalIPAttachment/NATGateway →
  ExternalIP, ExternalIP → ExternalIPPool, and Subnet/SecurityGroup/NATGateway
  → VirtualNetwork.
- Verify Catalog-created resources and direct-created resources have the same
  final network validation result, while Catalog metadata is not modified by
  networking resolution.

### End-to-end tests — supported user workflows

The E2E suite must run the following supported workflows in the deployment
topologies that advertise them:

- Provider creates one valid combined-manager NetworkClass and one valid
  K8s-only NetworkClass where applicable.
- Tenant creates a VirtualNetwork, IPv4 Subnet, SecurityGroup with valid
  rules, ExternalIPPool, ExternalIP, ExternalIPAttachment, and NATGateway
  where the selected manager supports each resource.
- Tenant reads and lists only its own resources and sees status transitions
  from Pending/Provisioning to Ready.
- VMaaS, CaaS, and BMaaS consume the same Ready Subnet and SecurityGroup
  according to their service-specific attachment cardinality.
- ExternalIPAttachment produces working inbound connectivity after the target
  IP and ExternalIP become ready.
- NATGateway produces the documented outbound source-IP behavior where it is
  supported.
- Verify two or more supported hosting clusters receive the required overlay
  for the same Subnet and that workloads on those clusters share the
  documented fabric segment.
- Verify VirtualNetworks remain isolated: workloads in different VNs cannot
  communicate through the networking API's supported path because VN peering
  is not implemented.
- Verify fabric-only BMaaS and BM-based CaaS workflows work without a K8s
  manager when their NetworkClass capabilities advertise those surfaces.
- Delete resources in leaf-first order and verify backend cleanup and
  capacity release.
- Restart controllers during provisioning and verify eventual completion
  without duplicates.
- Recover from a transient manager failure and verify the resource remains
  non-Ready until the backend is actually ready.

### End-to-end tests — explicitly unsupported behavior

The following must be represented by negative E2E tests where the behavior is
user-visible; unit/integration tests are sufficient for provider-only or
backend-only restrictions:

- IPv6 and dual-stack resource creation are rejected.
- Multi-CIDR ExternalIPPools are rejected; tenants must create separate pools.
- NATGateway creation in a K8s-only deployment without NAT capability is
  rejected.
- VM creation without a K8s manager is rejected.
- Compute or BM multi-interface requests are rejected, and CaaS per-node-set
  tenant network overrides are rejected.
- Update, patch, and replace of any network-owned spec field are rejected;
  delete and recreate is the supported change path.
- Cross-tenant, cross-project, cross-VirtualNetwork, non-Ready, and arbitrary
  address references are rejected.
- ExternalIP reuse by a second consumer is rejected.
- Unsafe parent deletion while dependencies exist is rejected.
- Tenant selection of provider-resolved manager, implementation strategy,
  physical interface, or controller status is rejected.
- Cross-VirtualNetwork communication/VN peering, multi-hub deployment,
  air-gapped deployment, and DNS API behavior are not presented as supported
  networking workflows and must not be accepted by this contract.
- East-west networking is deliberately excluded from this test plan. Its
  future update/resize behavior is not tested here and remains governed by
  its own design.

### Coverage and graduation gate

The release gate requires a traceability table mapping every normative rule in
the shared field contract, validation matrix, operation contract, and manager
capability matrix to at least one unit or integration test. Every user-visible
supported workflow must also have an E2E test, and every user-visible
unsupported workflow must have an E2E rejection test. A test is not complete
when it only observes an API error: it must also verify the documented error
class/condition and that no invalid object, allocation, backend operation, or
orphaned child remains.
